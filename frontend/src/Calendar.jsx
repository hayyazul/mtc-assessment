...
export default function Calendar() {
  ...
  const [gregMonth, setGregMonth]     = useState(today.getMonth());
  const [gregYear,  setGregYear]      = useState(today.getFullYear());
  const [ramadanData, setRamadanData] = useState([]);
  
  // When loading for the first time, get the location and the Ramadan timings.
  // This does not account for next year's Ramadan.
  useEffect(() => {
    ...
    fetchLocation().then(({ lat, lon }) => {
      fetch(`http://localhost:8000/ramadan?lat=${lat}&lon=${lon}`)
        .then((res) => res.json())
        .then((data) => setRamadanData(data))
        .catch((err) => console.log("Failed to fetch ramadan data:", err));
    });
  }, []);

  // Hijri navigation state — start from today's Hijri date
  ...
  const [primaryCalendar, setPrimaryCalendar] = useState("gregorian");
